...
class Amazons(object):
    ...
    def fire(self, player, location, itemtype):
        ...
        if player == 'A' and itemtype == 0:
            ...
            stateA = self.playerState['A']  # 复制A方棋子位置的引用
            stateA[stateA.index((itemfromY, itemfromX))] = (itemtoY, itemtoX)  # 改变A方棋子位置存储
            ...
            stateB = self.playerState['B']  # 这是复制B位置的引用
            stateB[stateB.index((itemfromY, itemfromX))] = (itemtoY, itemtoX)  # 改变B方棋子位置存储
